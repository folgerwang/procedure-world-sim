...
void Skydome::draw(
    ...
    float azimuth = glm::degrees(azimuth_angle_);
...
void Skydome::update() {
    float azimuth = glm::radians(azimuth_angle_);
    sun_dir_ = glm::vec3(
        glm::vec4(cos(azimuth), 0.0f, sin(azimuth), 0.0f) *
        glm::rotate(glm::mat4(1.0f),
            glm::radians(90.0f - zenith_angle_),
            glm::vec3(1.0f, 0.0f, 0.0f)));

    azimuth_angle_ += 0.01f;
}
